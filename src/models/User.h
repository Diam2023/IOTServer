...
class User
{
  public:
    ...
    /**  For column user_permission_level  */
    ///Get the value of the column user_permission_level, returns the default value if the column is null
    const uint32_t &getValueOfUserPermissionLevel() const noexcept;
    ///Return a shared_ptr object pointing to the column const value, or an empty shared_ptr object if the column is null
    const std::shared_ptr<uint32_t> &getUserPermissionLevel() const noexcept;
