...
void cq::CqMqttDeviceMessageHandler::handler(const mqtt::MqttData &data) {
    ...
    if (!result.first.empty()) {
        ...
        drogon::app().getLoop()->queueInLoop([deviceProm, result]() {
            ...
            deviceMapper.findOne(Criteria(Device::Cols::_device_sn, CompareOperator::EQ, result.first),
                                 [deviceProm](const Device &r) {
        ...
        drogon::app().getLoop()->queueInLoop([topicProm, result, deviceId]() {
            ...
            topicMapper.findOne(Criteria(Topic::Cols::_target_device_id, CompareOperator::EQ, deviceId)
                                && Criteria(Topic::Cols::_topic_name, CompareOperator::EQ, result.second),
                                [topicProm](const Topic &r) {
        ...
        drogon::app().getLoop()->queueInLoop([userIdListProm, deviceId, topicId]() {
            auto dbClientPtr = drogon::app().getDbClient();
            Mapper<SubscribeMap> subsMapper(dbClientPtr);
            subsMapper.findBy(Criteria(SubscribeMap::Cols::_target_device_id, CompareOperator::EQ, deviceId)
        ...
        auto sendVec = std::make_shared<std::vector<std::pair<std::string, std::string>>>();
        ...
        if (!sendVec->empty()) {
            for (const auto &res: *sendVec) {
                cq::CqMessageManager::getInstance().messageOut(std::get<0>(res), std::get<1>(res),
...
std::string cq::CqMqttDeviceMessageHandler::convertVisual(const mqtt::MqttData &data, const std::string &deviceName) {
    // Just For Test
    std::stringstream ss;
