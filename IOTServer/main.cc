 ...
 *
 */
static std::string configFile = "../../config.yaml";
...
int main(int argc, char **argv)
{
    ...
    QTimer::singleShot(500, &app, [&app]() {
        // 因为QMqttClient依赖于socket所以在不同线程中启动与发布会出现
        // qsocketnotifier: socket notifiers cannot be enabled or disabled from
        // another thread 问题
        // 但是QMqttClient在其他线程启动后不工作，status卡在Connecting也不抛出错误
        // 所以很头疼
        // https://forum.qt.io/topic/109138/qmqttclient-connection-not-made-from-another-thread
        // 方案不适用 最后解决方案Client在QtEventLoop中启动
        // Publish也在事件循环中调用
        mqtt::MqttClient::instance().registerNotifyCallback(
            cq::CqMqttDeviceMessageHandler::getInstance());
        mqtt::MqttClient::instance().registerNotifyCallback(
            DeviceMessageNotifyHandler::getInstance());
        mqtt::MqttClient::instance().registerNotifyCallback([](const auto &r) {
            // DEBUG
            LOG_INFO << "TOPIC: " << r.first.name().toStdString()
                     << "DATA: " << r.second.toStyledString();
        });

        // Load Mqtt Launch Mqtt start!
        mqtt::MqttClient::instance()
            .loadConfig(drogon::app().getCustomConfig()["mqtt"])
            .start();

        publisherWorkerPtr.reset(new QTimer(&app));

        QObject::connect(publisherWorkerPtr.data(),
                         &QTimer::timeout,
                         &mqtt::MqttMessagePublisher::getInstance(),
